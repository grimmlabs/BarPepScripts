...
## Analysis
python BarPepAnalysis.py -a BC -i example_data/assignment_file_for_analysis.BC.csv -l example_output/m1_input_Variants.csv -d example_output/ -x
...
- **-l, --BCVleft:**  
Right flanking sequence at 5' of the barcode/peptide location. Should be between 6 and 12 bp.  

- **-r, --BCVright:**  
Left flanking sequence at 3' of the barcode/peptide location. Should be between 6 and 12 bp.
...
- **-n, --BCVloc:**  
Position of the first expected barcode/peptide nucleotide if the read numbering starts with 0.
- **-m, --BCVmargin:**  
Number of nucleotides before and after BCV_loc to search for the barcode/peptide.
- **-k, --BCVlocrevcomp:**  
Position of the first expected barcode/peptide nucleotide on the reverse complement strand if the read numbering starts with 0.
...
- **PV file:**  
Table containing the detected peptide DNA sequences sorted by their frequency in the sequencing data.
...
In this mode the script requires the output from the _BarPepDetection_ script as well as one input file containing a table with your filenames, animals, tissues, and a weight variable (e.g. vg/dg). This file is easily prepared in Excel or a similar program and then saved as a CSV file. **The separator in the CSV file has to be a ','.**
...
**IMPORTANT:** The names of the columns have to be written exactly how it is shown here! Underscores are used for uniting and separating columns. Do not use a '_' in your animal description, sample type or tissue entries!
...
**IMPORTANT:** Underscores are used for uniting and separating columns. Do not use a '_' in your animal description, or tissue entries!
...
- **-l, --librarynorm:**  
 Path to the _Variants.csv output file of the input library from the _Detection Script_. This argument is required for barcode analysis.
...
- **Read Counts File:**  
Table of raw read counts of each variant in each tissue for each unique combination of animal and sample type. Additionally, it shows the vg/dg or RQ values as well as the input library normalization values. Each output file will follow this table structure.  
...
- **Ranking DNAseq File:**  
Table of the DNA sequences of the ranked top 100 (or otherwise specified number) peptides for each animal and tissue combination, their raw counts, and proportions.  

- **Ranking PeptideSeq File:**  
Table of the amino acid sequences of the ranked top 100 (or otherwise specified number) peptides for each animal and tissue combination, their raw counts, and proportions.
